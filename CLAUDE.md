...
Published to **Maven Central** with plugin id **`com.seanproctor.nucleus`**. Repo home: **https://github.com/sproctor/nucleus-plugin**.

> Note: the Kotlin DSL import packages remain `io.github.kdroidfilter.nucleus.*`. Only the Maven coordinates / plugin id use the `com.seanproctor` group.
...
- GraalVM reachability metadata ships as static JSON inside the plugin under `plugin/src/main/resources/nucleus/graalvm/` (per-library, platform-specific, and generic levels) — consumers do not copy hundreds of entries by hand
...
- Override the coordinates at publish time: group via `-PGROUP=com.seanproctor`, plugin id via `-PID=com.seanproctor.nucleus`
...
GITHUB_REF=refs/tags/v1.15.11 JAVA_HOME=/usr/lib/jvm/java-1.21.0-openjdk-amd64 \
  ./gradlew publishToMavenLocal -PGROUP=com.seanproctor -PID=com.seanproctor.nucleus

# Publish + release to Maven Central (requires signing + Sonatype credentials)
GITHUB_REF=refs/tags/v1.15.11 JAVA_HOME=/usr/lib/jvm/java-1.21.0-openjdk-amd64 \
  ./gradlew publishAndReleaseToMavenCentral -PGROUP=com.seanproctor -PID=com.seanproctor.nucleus \
...
- Reflection / reachability metadata is resolved and merged automatically at build time; users do not copy hundreds of entries:
    ...
    - **Platform-specific metadata** (macOS/Windows/Linux) shipped in the plugin under `plugin/src/main/resources/nucleus/graalvm/`
